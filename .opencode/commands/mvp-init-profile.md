---
description: Bootstrap project-profile (profile, constitution, standards, architecture) from conversation or existing code
---
...
**Input**: Optionally specify what you're building after `/mvp:init-profile`.
...
The artifacts live in the change folder during creation. After population,
copy them:
...
# domain-map.md only if created
```

The init-profile change can be archived — the artifacts in
`openspec/project/` persist independently.
...
- Constitution rules must be SPECIFIC and TESTABLE — no vague "write good code"
- Profile status MUST be auto-derived from filesystem scan, never fabricated
